            ...
            {data.body.map((section, sectionIndex) => (
                <div key={section.id} className="p-4 border border-gray-700 rounded-lg bg-gray-800/50">
                    <div className="flex items-center justify-between border-b-2 border-gray-700 pb-2 mb-4">
                         <div className="flex items-center flex-grow">
                            ...
                            {section.tituloEditavel ? (
                                <input 
                                    ...
                                    className="text-xl font-bold text-cyan-400 uppercase bg-gray-700 flex-grow p-1 rounded border border-gray-600 outline-none focus:border-cyan-500"
                        ...
                        {section.removivel && (
                             <button onClick={() => removeSection(section.id)} className="ml-4 text-red-500 hover:text-red-400 font-bold bg-gray-900 w-8 h-8 rounded flex items-center justify-center transition-colors">X</button>
                        )}
                    </div>
                                        ...
                                        <div className="space-y-4 mt-4">
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 border-b border-gray-700 pb-4">
                                                <div>
                                                    ...
                                                    <input type="text" value={data.assinaturas.propostoPor.nome} onChange={e => onSignatureChange('propostoPor', 'nome', e.target.value)} className="w-full mt-1 bg-gray-700 border border-gray-600 rounded-md py-2 px-3 text-white focus:border-cyan-500 outline-none" />
                                                </div>
                                                <div>
                                                    <span className="text-xs text-gray-400">Cargo:</span>
                                                    <input type="text" value={data.assinaturas.propostoPor.cargo} onChange={e => onSignatureChange('propostoPor', 'cargo', e.target.value)} className="w-full mt-1 bg-gray-700 border border-gray-600 rounded-md py-2 px-3 text-white focus:border-cyan-500 outline-none" />
                                                </div>
                                            </div>
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 border-b border-gray-700 pb-4">
                                                <div>
                                                    <span className="text-xs text-gray-400">Visto por (Nome):</span>
                                                    <input type="text" value={data.assinaturas.vistoPor.nome} onChange={e => onSignatureChange('vistoPor', 'nome', e.target.value)} className="w-full mt-1 bg-gray-700 border border-gray-600 rounded-md py-2 px-3 text-white focus:border-cyan-500 outline-none" />
                                                </div>
                                                <div>
                                                    <span className="text-xs text-gray-400">Cargo:</span>
                                                    <input type="text" value={data.assinaturas.vistoPor.cargo} onChange={e => onSignatureChange('vistoPor', 'cargo', e.target.value)} className="w-full mt-1 bg-gray-700 border border-gray-600 rounded-md py-2 px-3 text-white focus:border-cyan-500 outline-none" />
                                                </div>
                                            </div>
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                <div>
                                                    <span className="text-xs text-gray-400">Aprovado por (Nome):</span>
                                                    <input type="text" value={data.assinaturas.aprovadoPor.nome} onChange={e => onSignatureChange('aprovadoPor', 'nome', e.target.value)} className="w-full mt-1 bg-gray-700 border border-gray-600 rounded-md py-2 px-3 text-white focus:border-cyan-500 outline-none" />
                                                </div>
                                                <div>
                                                    <span className="text-xs text-gray-400">Cargo:</span>
                                                    <input type="text" value={data.assinaturas.aprovadoPor.cargo} onChange={e => onSignatureChange('aprovadoPor', 'cargo', e.target.value)} className="w-full mt-1 bg-gray-700 border border-gray-600 rounded-md py-2 px-3 text-white focus:border-cyan-500 outline-none" />
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div key={subsection.id} className="border-l-2 border-gray-600 pl-4 py-2">
                                    <div className="flex items-center justify-between mb-2">
                                        <div className="flex items-center flex-grow">
                                            ...
                                            {subsection.tituloEditavel ? (
                                                <input 
                                                    ...
                                                    className="text-sm font-bold text-gray-300 uppercase underline bg-gray-700 flex-grow p-1 rounded border border-gray-600 outline-none focus:border-cyan-500"
                                        ...
                                        <div className="flex space-x-2 ml-2">
                                            <button onClick={() => addSubSubSection(sectionIndex, subsectionIndex)} className="text-cyan-400 hover:text-cyan-300 font-bold text-[10px] bg-gray-700 px-2 py-1 rounded transition-colors">+ ALÍNEA</button>
                                            {subsection.removivel && (
                                                <button onClick={() => removeSubsection(sectionIndex, subsection.id)} className="text-red-500 hover:text-red-400 font-bold text-xs bg-gray-700 w-6 h-6 flex items-center justify-center rounded transition-colors">X</button>
                                            )}
                                        </div>
                                    </div>
                                                    ...
                                                    <div className="flex items-center justify-between mb-2">
                                                        <div className="flex items-center flex-grow">
                                                            ...
                                                            <input 
                                                                ...
                                                                className="text-xs font-bold text-gray-400 bg-gray-700 flex-grow p-1 rounded border border-gray-600 outline-none focus:border-cyan-500"
                                                                placeholder="Título da Alínea"
                                                            />
                                                        </div>
